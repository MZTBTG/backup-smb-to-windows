...
echo -e '==>   Backup de Windows para SMB automatizado  <==\n\n\033[05;31mCriado por MZTBTG!\033[m\n'
...
then
  ...
  mount //192.168.184.6/backup-clientes $bkp -o vers=1.0 # Edite alguns parâmetros dessa linha
  if [ $? -eq 0 ]
  then
    ...
    echo '|Coloque a senha logo em seguida.                                    |'
    echo "|                                                                    |"
    ...
    echo -e '\n\033[01;31mOcorreu um erro ao montar o backup.\033[m\n'
    umount $bkp
  fi
else
  echo -e "Modo de uso:\nDigite sudo $0 ano-mes-dia-nomeDoComputador sdxx"
  echo 'Caso você queira uma transferência de dados levemente mais rápida e limpa, digite "quiet" no final da sintaxe.'
fi
